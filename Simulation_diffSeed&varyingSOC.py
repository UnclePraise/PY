# Simulation_diffSeed%26varyingSOC.py

import random
import pulp as pl
import matplotlib.pyplot as plt
import numpy as np

# Define SoC ranges as (lower, upper) tuples
soc_ranges = [(0.20, 0.25), (0.20, 0.30), (0.25, 0.35), (0.25, 0.40), (0.05, 0.35)]
range_labels = [ "20-25%", "20-30%", "25-35%" ,"25-40%" , "5-35%"]  # Labels for the x-axis

n_buses = 10
battery_capacity = 230  # kWh
charging_window = 12
slot_duration = 0.5
max_rate_per_bus = 60
min_rate_per_bus = 20
CONTINUOUS_CHARGING = True

seeds_to_test = random.sample(range(1, 1000000), 13)  # 13 unique random seeds each run

fig_scatter, axs_scatter = plt.subplots(1, 3, figsize=(18, 5), sharey=True)
fig_box, axs_box = plt.subplots(1, 3, figsize=(18, 5), sharey=True)

for case_idx, (soc_ranges, range_labels, case_title) in enumerate(soc_cases):
    all_max_demand = []
    for soc_range in soc_ranges:
        lower_bound, upper_bound = soc_range
        max_demands = []
        for seed in seeds_to_test:
            random.seed(seed)
            arrival_soc = [random.uniform(lower_bound, upper_bound) for _ in range(n_buses)]
            energy_needed = [(1 - soc) * battery_capacity for soc in arrival_soc]

            model = pl.LpProblem("Bus_Charging_Optimization", pl.LpMinimize)
            x = pl.LpVariable.dicts("x", ((i, t) for i in range(n_buses) for t in range(charging_window)), cat="Binary")
            p = pl.LpVariable.dicts("p", ((i, t) for i in range(n_buses) for t in range(charging_window)), lowBound=0)
            peak_demand = pl.LpVariable("peak_demand", lowBound=0)
            model += peak_demand

            for i in range(n_buses):
                for t in range(charging_window):
                    model += p[i, t] <= max_rate_per_bus * x[i, t]
                    model += p[i, t] >= min_rate_per_bus * x[i, t]
            for i in range(n_buses):
                model += pl.lpSum(p[i, t] * slot_duration for t in range(charging_window)) >= energy_needed[i]
            for t in range(charging_window):
                model += pl.lpSum(p[i, t] for i in range(n_buses)) <= peak_demand
            if CONTINUOUS_CHARGING:
                for i in range(n_buses):
                    for t in range(charging_window - 1):
                        model += x[i, t] >= x[i, t + 1]

            solver = pl.PULP_CBC_CMD(msg=False)
            model.solve(solver)

            if pl.LpStatus[model.status] == "Optimal":
                demand_per_slot = [sum(p[i, t].varValue for i in range(n_buses) if p[i, t].varValue is not None) for t in range(charging_window)]
                optimum_max_demand = max(demand_per_slot)
                max_demands.append(optimum_max_demand / n_buses)  # Per bus

        all_max_demand.append(max_demands)

    # Scatter plot for this case
    ax = axs_scatter[case_idx]
    for idx, label in enumerate(range_labels):
        y = all_max_demand[idx]
        x = [label] * len(y)
        ax.scatter(x, y, s=20, alpha=0.7)
    ax.set_xlabel("Arrival SoC Range")
    ax.set_ylabel("Optimum Demand Charge per Bus (kW)")  # <-- Updated label with (kW)
    ax.set_title(case_title)
    ax.grid(True, linestyle='--', alpha=0.7)
    # Add a legend for kW unit
    ax.legend(["kW"], loc="upper right", fontsize=10, frameon=True)

    # Box plot for this case
    axb = axs_box[case_idx]
    axb.boxplot(all_max_demand, positions=range(len(range_labels)), widths=0.5, patch_artist=True,
                boxprops=dict(facecolor='lightblue', color='blue'),
                medianprops=dict(color='red', linewidth=2))
    axb.set_xlabel("Arrival SoC Range")
    axb.set_ylabel("Optimum Demand Charge per Bus (kW)")  # <-- Updated label with (kW)
    axb.set_title(case_title)
    axb.set_xticks(range(len(range_labels)))
    axb.set_xticklabels(range_labels)
    axb.grid(True, linestyle='--', alpha=0.7)
    # Add a legend for kW unit
    axb.legend(["kW"], loc="upper right", fontsize=10, frameon=True)

fig_scatter.suptitle("Optimum Demand Charge per Bus for Different Arrival SoC Ranges (Scatter)", fontsize=16)
fig_scatter.tight_layout(rect=[0, 0, 1, 0.96])
fig_box.suptitle("Box Plot: Optimum Demand Charge per Bus for Different Arrival SoC Ranges", fontsize=16)
fig_box.tight_layout(rect=[0, 0, 1, 0.96])
plt.show()
